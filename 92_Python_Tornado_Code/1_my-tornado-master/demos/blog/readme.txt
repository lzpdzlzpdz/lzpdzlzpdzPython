...
3. Connect to MySQL and create a database and user for the blog.
   ...
   Allow the "root" user to connect with the password "¡®password¡¯":
   mysql> GRANT ALL PRIVILEGES ON blog.* TO 'root'@'localhost' IDENTIFIED BY 'password';


4. Create the tables in your new database.

   You can use the provided schema.sql file by running this command:
   cmd>  cd E:\8_Git\Git_Python_Code\lzpdzlzpdzPython\92_Python_Tornado_Code\1_my-tornado-master\demos\blog
   cmd>  mysql --user=root --password=password --database=blog < schema.sql
