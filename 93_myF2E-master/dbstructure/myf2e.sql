...
DELIMITER ;;
CREATE TRIGGER `myf2e`.`image_delete_trigger`
BEFORE DELETE ON `myf2e`.`image`
FOR EACH ROW
BEGIN
...
DELIMITER ;;
CREATE TRIGGER `myf2e`.`topic_delete_trigger`
BEFORE DELETE ON `myf2e`.`topic`
FOR EACH ROW
BEGIN
